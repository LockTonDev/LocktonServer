...
module.exports = {
  ...
  selectList: async function (req, res, next) {
    try {
      ...
      const itemsData = await M_TCAA0040A.selectList(req);
      const countData = await M_TCAA0040A.selectCount(req);
  ...
  selectCount: async function (req, res, next) {
    try {
      ...
      const result = await M_TCAA0040A.selectCount(req);
  ...
  insert: async function (req, res, next) {
    try {
      ...
      const result = await M_TCAA0040A.insert(req);
  ...
  update: async function (req, res, next) {
    try {
      // if (!req.body.params) throw new BadRequest(StatusMessage.BadRequestMeg);
      const result = await M_TCAA0040A.update(req);
  ...
  delete: async function (req, res, next) {
    try {
      //if (!req.body.params) throw new BadRequest(StatusMessage.BadRequestMeg);
      const result = await M_TCAA0040A.delete(req);
