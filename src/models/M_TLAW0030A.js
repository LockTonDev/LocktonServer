...
module.exports = {
  ...
  update: async function (req) {
    const user_uuid = req.decoded.uuid;
    const params = req.body.params;
    const queryIND = ` UPDATE TLAW0030A 
                    SET 
                      insurance_no = ?,
                      business_cd = ?,
                      user_cd = ?,
                  ...
                  `;
    const queryJNT = ` UPDATE TLAW0030A 
                  SET 
                    insurance_no = ?,
                    business_cd = ?,
                    user_cd = ?,
  ...
  delete: async function (req) {
    const user_uuid = req.decoded.uuid;
    const queryIND = 'DELETE FROM TLAW0030A WHERE insurance_uuid = ? AND user_nm = ? and user_birth = ? and user_regno = ?';
    const queryJNT = 'DELETE FROM TLAW0030A WHERE insurance_uuid = ? AND user_nm = ? and corp_cnno = ?';
    const userQuery = `SELECT * from tcom0110a where user_uuid = ?`
    const [user] = await db.query(userQuery, user_uuid);  
    if (user.length < 1) {
      throw new NotFound(StatusMessage.SELECT_FAILED);
  ...
  updateFromUserInfo: async function (req) {
    const user_uuid = req.decoded.uuid;
    const params = req.body.params;
    const query = `UPDATE TLAW0030A SET corp_type=?, corp_nm=?, corp_ceo_nm=?, corp_bnno=?, corp_cnno=?, corp_telno=?, corp_faxno=?
                  , corp_cust_nm=?, corp_cust_hpno=?, corp_cust_email=?, corp_post=?, corp_addr=?, corp_addr_dtl=?, corp_region_cd=?
