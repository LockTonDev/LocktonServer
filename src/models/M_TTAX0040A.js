...
module.exports = {
  ...
  selectList: async function (req) {
    ...
    // const query = 'SELECT * FROM TTAX0040A WHERE user_id = ? ORDER BY apply_no DESC';
    let query = `SELECT a.*,
                        FN_GET_CODENM('COM040', a.apply_cd) AS apply_nm,
                        FN_GET_CODENM('COM041', a.proc_cd) AS proc_nm
                    FROM   TTAX0040A a
  ...
  selectCount: async function (req) {
    ...
    let query = 'SELECT COUNT(*) AS count FROM TTAX0040A WHERE user_uuid = ? ';
  ...
  insert: async function (req) {
    ...
    const query = `INSERT INTO TTAX0040A (user_cd, business_cd, insurance_no, insurance_user_nm, email,
  ...
  update: async function (req) {
    ...
    const query =
      'UPDATE TTAX0040A SET email = ?, tel = ?, nm = ?, apply_cd = ?, apply_content = ?, apply_posted_dt = ?, proc_at = ?, proc_content = ?, proc_cd = ? WHERE apply_no = ?';
    const {
      email,
      tel,
      tel1,
  ...
  delete: async function (req) {
    ...
    const query = 'DELETE FROM TTAX0040A WHERE apply_no = ?';
